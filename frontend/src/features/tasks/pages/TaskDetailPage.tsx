...
import { ArrowLeft, CheckCircle2, AlertTriangle, Check } from "lucide-react";
...
export const TaskDetailPage: React.FC = () => {
  ...
  const milestones =
    ((milestonesData as MilestonesResponse | undefined)?.milestones) ?? [];
              ...
              {milestones.length === 0 ? (
                <Card padding="md">
                  <p className="text-sm text-text-muted text-center py-4">
                    Milestones are being generated by SPARK AI...
                  </p>
                </Card>
              ) : (
                <div className="space-y-2">
                      ...
                      >
                        <div className="flex items-center gap-3 px-3 py-2">
                          ...
                          <button
                            onClick={() =>
                              !isCompleted &&
                              handleCompleteMilestone(ms.id)
                            }
                            disabled={isCompleted}
                            className={`w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 transition-all duration-200 ${
                              isCompleted
                                ? "bg-success border-success"
                                : "border-neutral-400 bg-neutral-50 hover:border-accent hover:bg-accent-light"
                            }`}
                            aria-label={
                              isCompleted
                                ? "Completed"
                                : `Complete milestone: ${ms.title}`
                            }
                          >
                            {isCompleted && (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {isNext && !isCompleted && (
                                <span className="text-xs text-accent font-semibold bg-accent-light px-2 py-0.5 rounded-full">
                                  Next action
                                </span>
                              )}
                            </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-text-primary">
                      {formatDuration(t.estimatedHours)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-text-muted">Progress</span>
                    <span className="text-text-primary">
                      {Math.round(t.progress?.percentage ?? 0)}%
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
